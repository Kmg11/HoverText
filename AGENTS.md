...
- `KeyboardHook.cs` — global low-level keyboard hook (`WH_KEYBOARD_LL`)
  raising `KeyDown`/`KeyUp` for a single VK code.
- `App.xaml.cs` — wires everything; a `DispatcherTimer` polls the cursor
  position every `PollIntervalMs` while the trigger key is held.
